    ...
    public partial class ModSDKWindow
    {
        private class UploadMod
        {
            ...
            public void OnEnable(VisualElement root)
            {
                ...
                IEnumerable<ModSettings> settingsAssets = AssetDatabase.FindAssets("t:PugMod.ModIO.ModSettings")
                ...
                _modIOSettings = new List<ModSettings>();

                foreach (var asset in settingsAssets)
                {
                    if (asset.modSettings == null)
                    {
                        string modBuilderSettingsName = asset.name.Replace("_modio","");

                        Debug.LogWarning($"{asset.name} has no ModBuilderSetting. Attempting to find default with name {modBuilderSettingsName}");
                        asset.modSettings = _modSettings.FirstOrDefault(x=> x.name == modBuilderSettingsName);
                        Assert.IsNotNull(asset.modSettings);
                    }
                    _modIOSettings.Add(asset);
                }
